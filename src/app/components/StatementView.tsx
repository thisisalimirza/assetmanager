...
import { getClientStatementData, getClientPeriodStatement, type PeriodStatement } from "@/lib/portfolio";
...
// Server-rendered statement building blocks, shared between the advisor's
// statement page (/clients/[id]/statement) and a client's read-only share link
// (/share/c/[token]) — both render the exact same artifact.

export function isoDate(d: Date): string {
  return d.toISOString().slice(0, 10);
}

export function statementRanges(): { key: string; label: string; from: string; to: string }[] {
  const now = new Date();
  ...
  return [
    { key: "month", label: "This month", from: isoDate(new Date(Date.UTC(y, m, 1))), to },
    { key: "quarter", label: "This quarter", from: isoDate(new Date(Date.UTC(y, q, 1))), to },
    { key: "ytd", label: "Year to date", from: isoDate(new Date(Date.UTC(y, 0, 1))), to },
  ];
}
...
export function RangeChips({
  ...
  return (
    <div className="flex flex-wrap gap-2 print:hidden">
      <RangeChip href={basePath} active={!period}>
        All time
      </RangeChip>
      {statementRanges().map((r) => (
        <RangeChip
          key={r.key}
...
export async function AllTimeStatementCard({ id, generatedOn }: { id: number; generatedOn: string }) {
...
export function PeriodStatementCard({
  period,
  generatedOn,
}: {
...
/**
 * Resolves the from/to search params into a period statement (or null for
 * all-time), 404ing on a bogus range — shared by both statement surfaces.
 */
export async function resolvePeriod(
  ...
  const period = await getClientPeriodStatement(id, from, to);
  if (!period) notFound();
  return period;
}
