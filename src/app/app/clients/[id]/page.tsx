...
export default async function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <PageHeader
        title={client.name}
        subtitle={[client.email, client.phone].filter(Boolean).join(" · ") || "No contact details"}
        actions={
          <>
      ...
      <div>
        <SectionHeader>Statements &amp; sharing</SectionHeader>
        <div className="grid gap-3 lg:grid-cols-2">
          <div className={`${cardStyles} flex flex-col`}>
            <h3 className="text-sm font-medium">Statement &amp; export</h3>
            <p className="mt-0.5 flex-1 text-xs text-zinc-500">
              A printable statement (all-time or by period), and a CSV of every cash flow with
